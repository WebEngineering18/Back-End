...
/**
 * Controller zum Speichern der Antworten einer fertigen Umfrage in der Datenbank
 * und zum Downloaden der Excel-Datei.
 *
...
public class UmfrageController {
    ...
    /**
     * Verarbeitet das JSONArray mit den Antworten einer Umfrage und speichert
     * diese in der Datenbank.<br>
     * <br>
     * Das gesamte JSONArray wird durch iteriert und jede Antwort wird mit der
     * saveAnswer()-Methode gespeichert in der Answers und in der
     * Answer_Question Tabelle gespeichert.
     *
     * @param answer JSONArray, welches alle Antworten einer fertigen Umfrage
     * beinhaltet
     * @param request wird gebraucht um aus dem HTTP Request die IP-Adresse des
     * Users zu bekommen
     * @return True wenn es eine neuer User beziehungsweise eine neue SessionId ist, sonst false
     */
    @CrossOrigin(origins = "http://localhost:8080")
    @PostMapping(produces = MediaType.APPLICATION_JSON_VALUE, consumes = "application/json", value = "/saveAnswers")
    public boolean create(@RequestBody final String answer, HttpServletRequest request) {
        
        boolean state = false;
        
        if (!isSameSessionId(request)) {
     ...
     * @return 
     */
    private boolean isSameSessionId(HttpServletRequest request) {

        List<User> userList = userRepository.findAll();
        boolean session = false;

        for (User u : userList) {
            if (u.getSessionId().equals(request.getRequestedSessionId())) {
                session = true;
                break;
            }
        }
